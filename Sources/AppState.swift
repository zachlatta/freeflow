...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    /// Which read method produced the last dictation's surrounding text (e.g. axAPI, keyboardSimulation), for the debug panel.
    ...
    // context is ready by the time recording stops. Non-disruptive (no keyboard simulation).
    ...
    private func startRecording(triggerMode: RecordingTriggerMode) {
        ...
        // without needing any Left/Right arrow hacks.
        Task { @MainActor in
    ...
    /// keyboard simulation, no screenshot, no LLM — those heavier paths stay in the
    ...
    private func endCriticalDictationActivity() {
        ...
        // Capture the in-flight transcription so the keepalive is NOT torn down while the paste
        // pipeline (and its paste-time context re-read) is still running — under CPU load that
        // work can outlast the fixed 1.5s, and an early teardown would blind the read.
        let pasteFlow = transcriptionTask
        Task { @MainActor [weak self] in
            try? await Task.sleep(nanoseconds: 1_500_000_000)
            _ = await pasteFlow?.value
            // A newer dictation may have started and re-armed the keepalive while we waited;
            // its own teardown owns the tree now, so a stale teardown here must NOT sleep it.
        ...
        let postProcessingService = PostProcessingService(
            ...
            self.transcriptionTask = Task {
                ...
                do {
                    ...
                    // Keyboard simulation is a TRUE last resort: it runs only when the stop read is
                    // blind AND the non-destructive start-of-recording snapshot is ALSO blind. When the
                    // start snapshot has text we SKIP the keyboard read and let the start-of-recording
                    // fallback below apply it (carrying its own method/appKind, so the Run Log stays
                    // faithful to how the text was actually read). The start snapshot is still valid —
                    // the preceding text doesn't change during dictation — and reading it never moves
                    // the caret.
                    let startSnapshotForGate = await startSurroundingHandle?.value
                    let startSnapshotBlind = !((startSnapshotForGate?.precedingText?.contains(where: { !$0.isWhitespace }) ?? false)
                        || (startSnapshotForGate?.followingText?.contains(where: { !$0.isWhitespace }) ?? false))

                    if finalCursorPos == "unknown", startSnapshotBlind {
                        let hasSelection = !(appContext.selectedText?.isEmpty ?? true)
                        // The Backspace below clears the selected range so the read sees clean
                        // surrounding text, turning the paste into a pure insertion. We only do
                        // this for standard Mac apps. In web views (browsers/Electron) selection
                        // detection is unreliable: a phantom selection would make this delete a
                        // real character — often a line/paragraph break — corrupting the document.
                        // Cmd+V already replaces a genuine selection there, so the delete is both
                        // unnecessary and risky; we skip it and keep the selection for Cmd+V.
                        let isWebView = (appContext.appKind == "webView")
                        let shouldClearSelection = hasSelection && !isWebView
                        os_log(.info, log: recordingLog,
                               "JIT re-read triggered: cursorPos=unknown hasSelection=%{public}@ webView=%{public}@",
                               hasSelection ? "true" : "false", isWebView ? "true" : "false")

                        // Re-verify a LIVE selection at paste time before the destructive Backspace:
                        // the recording-time selectedText can be stale (the user deselected), and a
                        // Backspace then would delete a real character instead of clearing a selection.
                        let liveSelection = NSWorkspace.shared.frontmostApplication.map {
                            AccessibilityTextReader.hasLiveSelection(in: AXUIElementCreateApplication($0.processIdentifier))
                        } ?? false

                        if shouldClearSelection && liveSelection {
                            // Send Backspace to clear the selection so we can read the exact cursor context.
                            ...
                            os_log(.info, log: recordingLog, "JIT: sent Backspace (code 51) to clear live selection")
                            // Allow OS to process deletion before attempting to read context
                            try? await Task.sleep(nanoseconds: 60_000_000)
                        } else if shouldClearSelection {
                            os_log(.info, log: recordingLog, "JIT: skipped Backspace — no live selection at paste time")
                        }
                        ...
                        if let frontmostApp = NSWorkspace.shared.frontmostApplication {
                            ...
                            while attempts < maxAttempts && !didFindContext {
                                try Task.checkCancellation()
                                ...
                                if surrounding.hasContext && (!isBlind || hasSelection) {
                            ...
                            if !didFindContext {
                                os_log(.error, log: recordingLog,
                                       "JIT failed after %d attempts — using stop-time context (may be blind app)", maxAttempts)
                            }
                        }
                    ...
                    let pasteSnap: SurroundingTextSnapshot? = NSWorkspace.shared.frontmostApplication.map {
                        AccessibilityTextReader.readSurroundingText(from: AXUIElementCreateApplication($0.processIdentifier))
                    }
                    ...
                    await MainActor.run {
                        ...
                        var finalPreceding = startFallback?.preceding ?? resolvedPreceding
                        var finalFollowing = startFallback?.following ?? resolvedFollowing
                        var finalCursor    = startFallback?.cursor   ?? resolvedCursorPos
                        var finalMethod    = startFallback?.method   ?? appContext.contextExtractionMethod
                        var finalAppKind   = startFallback?.appKind  ?? appContext.appKind
                        ...
                        if let pasteSnap {
                            ...
                            if (finalAppKind == nil || finalAppKind == "unknown"),
                               pasteSnap.appKind != .unknown {
                                finalAppKind = pasteSnap.appKind.rawValue
                            }
                            ...
                            let isNativeApp = (finalAppKind == "native")
                            if pasteSnap.hasContext, isNativeApp {
                                finalPreceding = pasteSnap.precedingText
                                finalFollowing = pasteSnap.followingText
                                finalCursor    = pasteSnap.cursorPosition.rawValue
                                finalMethod    = pasteSnap.extractionMethod.rawValue
                                os_log(.info, log: recordingLog,
                        ...
                        finalPreceding = finalPreceding.map(ContextualFormattingService.normalizeLineBreaks)
                        finalFollowing = finalFollowing.map(ContextualFormattingService.normalizeLineBreaks)

                        // Publish the resolved context to the live debug panel.
                        self.lastContextPrecedingText = finalPreceding ?? ""
                        self.lastContextFollowingText = finalFollowing ?? ""
                        self.lastContextCursorPosition = String(describing: finalCursor ?? "unknown")
                        self.lastContextExtractionMethod = finalMethod
                        // Extraction-ladder result: which rung resolved the context (method == rung).
                        // axWebTextMarker/axAPI/axWebAreaBFS = non-destructive; keyboardSimulation =
                        // the web/Electron last resort (should be rare and never at release).
                        os_log(.info, log: recordingLog, "context ladder: resolved via %{public}@",
                               finalMethod ?? "blind")
                        // Session metric: how often precise reads win vs the copy/paste fallback.
                        os_log(.info, log: recordingLog, "context-read metric: %{public}@",
                               ContextReadMetrics.record(finalMethod))
                        self.lastContextAppKind = finalAppKind
                        ...
                        self.lastIsBlindApp = (finalPreceding == nil && finalFollowing == nil)

                        let formatResult = ContextualFormattingService.format(
                            trimmedFinalTranscript,
                            precedingText: finalPreceding,
                            followingText: finalFollowing,
                        ...
                        resolvedContext.precedingText           = finalPreceding
                        resolvedContext.followingText           = finalFollowing
                        resolvedContext.cursorPosition          = finalCursor
                        resolvedContext.contextExtractionMethod = finalMethod
                        resolvedContext.appKind                 = finalAppKind
                        self.recordPipelineHistoryEntry(
                            ...
                            cursorPosition: self.lastContextCursorPosition,
                            contextFormatRule: formatResult.ruleApplied,
                            isBlindApp: self.lastIsBlindApp
                        )
    ...
    private func startContextCapture() {
        ...
        contextCaptureTask = Task { [weak self] in
            // Delay removed because capture now happens at the END of dictation (JIT),
            // so the OS tree is already fully stabilized.
