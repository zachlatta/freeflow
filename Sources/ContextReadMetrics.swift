...
///
/// Across a session this shows how often the precise accessibility paths win
/// versus the copy/paste fallback (which momentarily borrows the clipboard).
...
enum ContextReadMetrics {
    ...
    static func record(_ method: String?) -> String {
        ...
        let key = (method?.isEmpty == false) ? method! : "unknown"
        tally[key, default: 0] += 1

        let total    = tally.values.reduce(0, +)
        let fallback = tally["keyboardSimulation"] ?? 0   // copy/paste fallback
        let failed   = tally["unknown"] ?? 0              // couldn't read at all
        let precise  = total - fallback - failed          // accessibility paths

        return "\(total) reads this session · \(precise) precise · \(fallback) copy/paste fallback · \(failed) unreadable"
    }
}
